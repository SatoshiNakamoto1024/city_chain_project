                ...
                if length == -1:  # indef length
                    for value in concreteDecoder.indefLenValueDecoder(
                        ...
                        if isinstance(value, SubstrateUnderrunError):
                            yield value
                            # 🔽 ここに追加！
                            print(f"[DEBUG] decoded value for tagSet={tagSet}: type={type(value)}, hasValue={getattr(value, 'hasValue', lambda: None)()}, isNoValue={value is noValue}")
                else:
                    for value in concreteDecoder.valueDecoder(
                        ...
                        if isinstance(value, SubstrateUnderrunError):
                            yield value
                            # 🔽 ここに追加！
                            print(f"[DEBUG] decoded value: {value}, type: {type(value)}, hasValue: {getattr(value, 'hasValue', lambda: None)()}")
