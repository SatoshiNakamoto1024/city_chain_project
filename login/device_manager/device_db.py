...
def delete_device(user_uuid: str, device_id: str):
    ...
    """
    try:
        device_table.delete_item(Key={"uuid": user_uuid, "device_id": device_id})
...
def get_primary_session(user_uuid: str):
    """
    UsersTable 側にあるプライマリセッション情報を取得
    """
    try:
...
def delete_primary_session(user_uuid: str, session_id: str):
    """
    UsersTable 側のセッションを削除
    """
    try:
        users_table.delete_item(Key={"uuid": user_uuid, "session_id": session_id})
