# File: client_code/android/app_flask.py
"""
...
from user_manager.utils.login_handler import device_auth_bp      # ★追加
from user_manager.utils.app_utils     import utils_bp   
from user_manager.app_user_manager import user_bp  # optional

app = Flask(__name__)

# Blueprint 登録（順序が大事です）
app.register_blueprint(register_bp,  url_prefix="/register")   # 既存
app.register_blueprint(auth_bp)                                # 既存
app.register_blueprint(device_auth_bp, url_prefix="/device_auth")  # ★追加
app.register_blueprint(utils_bp,     url_prefix="/utils")          # ★追加
app.register_blueprint(user_bp,      url_prefix="/user")       # 既存
