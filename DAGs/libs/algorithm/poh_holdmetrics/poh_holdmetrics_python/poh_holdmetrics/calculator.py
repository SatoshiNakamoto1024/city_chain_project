...
try:
    if os.getenv("POH_DISABLE_RUST"):
        raise ImportError("disabled via POH_DISABLE_RUST")
    import poh_holdmetrics_rust as _rust  # type: ignore
except Exception:  # ImportError 含む
    _rust = None
    _HAS_RUST = False
    _logger.warning("Rust extension not used; falling back to pure-Python calculate_score()")
else:
    _HAS_RUST = True
