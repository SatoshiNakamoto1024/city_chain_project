...
app = FastAPI(title="Receiving-DAG Gateway")

dag = DAGStore()
rm  = RepairManager(dag, Path("pem/client.pem"))

@app.post("/repair")
def repair(missing_tx_id: str, requester: str):
    rm.request_repair(missing_tx_id, requester)
