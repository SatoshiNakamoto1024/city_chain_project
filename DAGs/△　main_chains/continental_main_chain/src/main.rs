...
use immudb::client::ImmudbClient;
use prost::bytes::Bytes;
//immuDB 関連の import を整理 + ウォレット管理用ヘルパーを追加
...
/// ProofOfHistory: イベント連鎖ハッシュを保持する簡易的な実装例
...
impl ProofOfHistory {
    ...
    /// 新たなイベント（例えば "TxID:Timestamp" など）をPoHへ追加する
    fn add_event(&mut self, event_str: &str) {
        let mut hasher = Sha256::new();
        // 以前のcurrent_hashと新しいeventを連結してハッシュ化
...
async fn connect_to_immudb(host: &str, port: u16) -> Result<ImmuServiceClient<Channel>, Box<dyn std::error::Error>> {
    ...
    // immuDBへのログインロジックが必要ならここで
    Ok(client)
}

/// immuDBに user_id のウォレット残高を保存（上書き）
async fn save_wallet_to_immudb(user_id: &str, balance: f64, host: &str, port: u16)
    -> Result<(), Box<dyn std::error::Error>>
{
    let mut client = connect_to_immudb(host, port).await?;
    let key = format!("{}{}", IMMUDB_WALLET_PREFIX, user_id).into_bytes();
    let value = balance.to_string().into_bytes();

    let kv = KeyValue { key, value };
    let request = tonic::Request::new(SetRequest { kvs: vec![kv] });
    client.set(request).await?;
    println!("[immuDB] user_id={} balance={} saved.", user_id, balance);
    Ok(())
}

/// immuDBから user_id のウォレット残高を取得（存在しなければ0）
async fn get_wallet_from_immudb(user_id: &str, host: &str, port: u16)
    -> Result<f64, Box<dyn std::error::Error>>
{
    ...
    let resp = client.get(tonic::Request::new(req)).await?;
    let wallet_bytes = resp.into_inner().value;
    let balance_str = String::from_utf8(wallet_bytes)?;
    Ok(balance_str.parse::<f64>().unwrap_or(0.0))
}

/// immuDB上の user_id ウォレット残高に delta を加減算して更新
async fn update_wallet_in_immudb(user_id: &str, delta: f64, host: &str, port: u16)
    -> Result<f64, Box<dyn std::error::Error>>
{
    let current_balance = get_wallet_from_immudb(user_id, host, port).await?;
    let new_balance = if current_balance + delta < 0.0 {
        0.0
    } else {
        current_balance + delta
    };
    save_wallet_to_immudb(user_id, new_balance, host, port).await?;
    println!("[immuDB] user_id={} new_balance={} after delta={}", user_id, new_balance, delta);
    Ok(new_balance)
}

// MongoDBのwallet_collection (必要であれば)
lazy_static! {
    static ref WALLET_CLIENT: MongoClient = {
        let client = MongoClient::with_uri_str("mongodb://localhost:27017").expect("Failed to connect to local Mongo");
        client
    };
}
static WALLET_DB_NAME: &str = "wallet_db";
static WALLET_COLL_NAME: &str = "wallets";

fn get_mongo_wallet_collection() -> Collection<Document> {
    WALLET_CLIENT
        .database(WALLET_DB_NAME)
        .collection::<Document>(WALLET_COLL_NAME)
}

/// MongoDB + immuDB の二重管理 (サンプルで "大きい方" を返す例)
async fn get_wallet_balance_both(user_id: &str, host: &str, port: u16) -> f64 {
    // MongoDB 側
    let coll = get_mongo_wallet_collection();
    let filter = doc! {"user_id": user_id};
    let doc = coll.find_one(filter, None)
        .expect("MongoDB query error (wallet)")
        .unwrap_or_else(|| doc!{"user_id": user_id, "balance": 0.0});
    let mongo_balance = doc.get_f64("balance").unwrap_or(0.0);

    // immuDB 側
    let immu_balance = match get_wallet_from_immudb(user_id, host, port).await {
        Ok(b) => b,
        Err(e) => {
            println!("Error in get_wallet_from_immudb: {:?}", e);
            0.0
        }
    };

    let final_balance = mongo_balance.max(immu_balance);
    println!("[Both] user_id={} => mongo_balance={}, immu_balance={}, final={}", user_id, mongo_balance, immu_balance, final_balance);
    final_balance
}

/// MongoDB + immuDB の二重管理で残高を更新
async fn update_wallet_balance_both(user_id: &str, amount: f64, host: &str, port: u16) -> f64 {
    // 1) MongoDB更新
    let coll = get_mongo_wallet_collection();
    let filter = doc! { "user_id": user_id };
    let doc_opt = coll.find_one(filter.clone(), None).expect("Mongo query err");
    let new_mongo_balance = if let Some(d) = doc_opt {
        let old_bal = d.get_f64("balance").unwrap_or(0.0);
        let mut new_bal = old_bal + amount;
        if new_bal < 0.0 { new_bal = 0.0; }
        coll.update_one(filter.clone(), doc!{"$set": {"balance": new_bal}}, None).expect("Mongo update error");
        new_bal
    } else {
        // Insert new doc
        let init_bal = if amount < 0.0 {0.0} else {amount};
        coll.insert_one(doc!{"user_id": user_id, "balance": init_bal}, None).expect("Mongo insert error");
        init_bal
    };

    // 2) immuDB 更新
    let new_immu_balance = match update_wallet_in_immudb(user_id, amount, host, port).await {
        Ok(b) => b,
        Err(e) => {
            println!("Error in update_wallet_in_immuDB: {:?}", e);
            0.0
        }
    };
    let final_balance = new_mongo_balance.max(new_immu_balance);
    println!("[Both] user_id={}, after amount={}, mongo={}, immu={}, final={}", user_id, amount, new_mongo_balance, new_immu_balance, final_balance);
    final_balance
}
...
impl Block {
    pub fn create_new_block(
        ...
        // ハッシュの生成
        let hash_input = format!("{}{}{}{}", index, prev_hash, timestamp, data);
        let hash = format!("{:x}", md5::compute(hash_input));

        // NTRU署名の生成
        let signature_vec = ntru_sign::sign(hash.as_bytes(), private_key)
        ...
        Ok(Block {
            ...
            hash,
            verifiable_credential: verifiable_credential.to_string(),
            signature,
        })
    }
}
...
struct Transaction {
    ...
    attributes: HashMap<String, String>, // 新しく追加
}
...
struct AppState {
    ...
    other_continental_chains: Arc<Mutex<Vec<String>>>,
    mongo_collection: Collection<Transaction>,
    block_collection: Collection<Document>,
    blockchain: Arc<Mutex<Vec<Block>>>,
    municipal_chain_urls: HashMap<String, String>,
    dpos: Arc<Mutex<DPoS>>,
    // Dilithium鍵を保持するフィールドを追加
    dilithium_public_key: DilithiumPublicKey,
    dilithium_secret_key: DilithiumSecretKey,
    // PoHを保持する
    poh: Arc<Mutex<ProofOfHistory>>,
}

impl AppState {
    /// ContinentalMainChainで使う署名を返す
    pub fn sign_data(&self, message: &[u8]) -> Vec<u8> {
        dilithium_sign(message, &self.dilithium_secret_key)
    }

    /// 署名検証
    pub fn verify_data(&self, message: &[u8], signature: &[u8]) -> bool {
        match dilithium_verify(signature, message, &self.dilithium_public_key) {
            Ok(()) => true,
            Err(_) => false,
        }
    }
...
impl Transaction {
    /// トランザクションデータをNTRUで暗号化
    pub fn encrypt_transaction_data(&mut self, ntru_public_key: &[u8]) -> Result<(), String> {
        let encryptor = NtruEncrypt::from_bytes(ntru_public_key)
            .map_err(|e| format!("Failed to initialize NTRU encryptor: {}", e))?;
        let encrypted = encryptor
            .encrypt(self.data.as_bytes())
            .map_err(|e| format!("Failed to encrypt data: {}", e))?;
        self.encrypted_data = base64::encode(encrypted);
        Ok(())
    }
    ...
    pub fn decrypt_transaction_data(
        ...
        let decryptor = NtruEncrypt::from_bytes(ntru_private_key)
            .map_err(|e| format!("Failed to initialize NTRU decryptor: {}", e))?;
        let decoded_data = base64::decode(encrypted_data)
            .map_err(|e| format!("Failed to decode encrypted data: {}", e))?;
        let decrypted = decryptor
            .decrypt(&decoded_data)
            .map_err(|e| format!("Failed to decrypt data: {}", e))?;
        String::from_utf8(decrypted).map_err(|e| format!("Failed to parse decrypted data: {}", e))
    }

    /// トランザクションの署名をDilithiumで生成
    pub fn generate_signature(&mut self, dilithium_private_key: &[u8]) -> Result<(), String> {
        let signer = NtruSign::from_bytes(dilithium_private_key)
            .map_err(|e| format!("Failed to initialize Dilithium signer: {}", e))?;
        let signature = signer
            .sign(self.data.as_bytes())
            .map_err(|e| format!("Failed to generate signature: {}", e))?;
    ...
    pub fn verify_signature(
        ...
        let verifier = NtruSign::from_bytes(dilithium_public_key)
            .map_err(|e| format!("Failed to initialize Dilithium verifier: {}", e))?;
        let decoded_signature = base64::decode(signature)
            .map_err(|e| format!("Failed to decode signature: {}", e))?;
        verifier
            .verify(data.as_bytes(), &decoded_signature)
            .map_err(|e| format!("Failed to verify signature: {}", e))
    }

    /// Proof of History (時系列証明) を生成
...
async fn receive_transaction(
    ...
    let signature_bytes = match base64::decode(&transaction.signature) {
        Ok(b) => b,
        Err(_) => {
            return Err("Signature decode failed".to_string());
        }
    };

    // トランザクション構造体をJSONに戻す or まとめる
    ...
    // 署名検証
    if !locked_state.verify_data(&tx_data, &signature_bytes) {
    ...
    // 2. 暗号化されたトランザクションデータを復号化
    let decrypted_data = match Transaction::decrypt_transaction_data(
        &transaction.encrypted_data,
        &state.lock().await.ntru_private_key,
    ) {
        Ok(data) => data,
        Err(e) => {
            println!("Failed to decrypt transaction data: {}", e);
            return Err("Failed to decrypt transaction data.".to_string());
        }
    };

    println!("Decrypted transaction data: {}", decrypted_data);
    ...
    if !Transaction::verify_signature(
        ...
        &state.lock().await.dilithium_public_key,
    )
    .unwrap_or(false)
    {
    ...
    if receiver_continent != current_continent {
        ...
        if transaction.status == "approved" {
            // 受信者の user_id
            let receiver_user_id = transaction.receiver.clone();
            // amount
            let amt = transaction.amount;

            // immuDB の host/port を読み込み
            let config = load_immudb_config().expect("Failed to load immuDB config");
            let (immu_host, immu_port) = get_immudb_address(&config, &transaction.receiver_continent);

            // MongoDB + immuDB の二重管理
            let final_bal = update_wallet_balance_both(&receiver_user_id, amt, &immu_host, immu_port).await;
            println!("Receiver {} new balance = {}", receiver_user_id, final_bal);
        }
...
fn validate_transaction(transaction: &Transaction) -> Result<(), String> {
    ...
    // attributesの検証（例: 空のキーがないか）
    for (key, value) in &transaction.attributes {
        if key.trim().is_empty() {
            return Err("Attribute key cannot be empty.".to_string());
        }
        // 必要に応じて値の検証も追加
        if value.trim().is_empty() {
            return Err(format!("Attribute value for key '{}' cannot be empty.", key));
        }
    }
...
async fn process_pending_transactions(state: Arc<Mutex<AppState>>) {
    loop {
        ...
        
        {
            ...
            let mut state_guard = state.lock().await;
            let mut poh_guard = state_guard.poh.lock().await;
            let event_str = format!("block_index:{}|hash:{}", new_block.index, new_block.hash);
            poh_guard.add_event(&event_str);
            println!("PoH updated by block => latest_hash={}", poh_guard.get_latest_hash());
        }
...
async fn get_poh_status(state: &State<Arc<Mutex<AppState>>>) -> Json<Value> {
    ...
    let events = poh_guard.get_events(); // 全イベント（長くなる場合がある点は注意）

    Json(serde_json::json!({
        "latest_poh_hash": latest,
        "event_count": count,
        "events": events
    }))
}
...
// 例: 特定の属性を持つトランザクションを検索する関数
async fn get_transactions_by_attribute(
    key: &str,
    value: &str,
...
async fn create_transaction(
    ...
    let bson_timestamp = BsonDateTime::from_millis(
        transaction.timestamp.parse::<DateTime<Utc>>().unwrap().timestamp_millis(),
    );
    let bson_created_at = BsonDateTime::from_millis(transaction.created_at.timestamp_millis());
...
async fn update_status(
    ...
    if let Err(e) = transaction {
        println!("Failed to find transaction: {:?}", e);
        return Err("Transaction not found.".to_string());
    }

    if let Some(current_transaction) = transaction.unwrap() {
        let current_status = current_transaction.get_str("status").unwrap_or("");

        // 状態遷移の検証
        if current_status != "pending" && new_status == "complete" {
            println!("Invalid status transition: {} -> {}", current_status, new_status);
            return Err("Invalid status transition.".to_string());
        }
        ...
        match state_guard.mongo_collection.update_one(filter.clone(), update, None).await {
            ...
            Err(e) => {
                ...
                Err("Failed to update transaction.".to_string())
            }
        }
    } else {
        println!("Transaction not found for id: {}", transaction_id);
        ...
        if !pending_transactions.contains_key(tx_id) {
            ...
            for (key, value) in &transaction.attributes {
                if key.trim().is_empty() || value.trim().is_empty() {
                    println!("Invalid attribute in transaction {}: key or value is empty.", tx_id);
                    continue; // 無効な属性をスキップ
                }
            }
...
//　walletの残高計算
pub fn calculate_wallet_balance(wallet_address: &str) -> Result<i64, Box<dyn Error>> {
    // immuDBクライアントの初期化
    let client = ImmudbClient::connect("localhost", 9497)?;
    ...
    for tx in transactions {
        ...
        } else if tx_data.from_wallet == wallet_address {
            balance -= tx_data.amount;
        }
    }
...
async fn main() {
    ...
    // 2) AppStateを作るときに詰め込む
    let state = Arc::new(Mutex::new(AppState {
        // ...
        dilithium_public_key: dil_pk,
        dilithium_secret_key: dil_sk,
        // ...
    }));
    ...
    match calculate_wallet_balance(wallet_address) {
        Ok(balance) => {
            println!("Wallet {} has a balance of {}", wallet_address, balance);
        },
        Err(e) => {
...
mod tests {
    ...
    async fn test_update_status_shared() {
        ...
        let transaction = Transaction {
            ...
            attributes: "attributes".attributes.clone(), // 新しく追加
        };
    ...
    #[tokio::test]
    async fn test_create_transaction_with_attributes() {
        // MongoDBクライアントの初期化
        let mongo_client = MongoClient::with_uri_str("mongodb://localhost:27017")
            .await
        ...
        let rocket = rocket::build()
            ...
            .manage(Arc::new(Mutex::new(AppState {
                ...
                dpos: Arc::new(Mutex::new(DPoS::new())), // DPoSの初期化
            })))
        ...
        let transaction = serde_json::json!({
            ...
            "transaction_id": "test_tx_id_with_attributes",
            ...
            "receiver_municipal_id": "receiver_municipal_id",
            "attributes": {
                "project_id": "12345",
                "category": "マーケティング",
                "description": "新製品の広告費"
        ...
        // MongoDBに保存されたトランザクションを確認
        let saved_doc = mongo_collection.find_one(doc! { "transaction_id": "test_tx_id_with_attributes" }, None).await.unwrap();
        assert!(saved_doc.is_some());

        if let Some(doc) = saved_doc {
            // attributesフィールドが正しく保存されているか確認
            if let Some(attributes) = doc.get_document("attributes").ok() {
                assert_eq!(attributes.get_str("project_id").unwrap(), "12345");
                assert_eq!(attributes.get_str("category").unwrap(), "マーケティング");
                assert_eq!(attributes.get_str("description").unwrap(), "新製品の広告費");
            } else {
                panic!("Attributes field is missing or not a document.");
            }
        } else {
            panic!("Transaction document was not found in MongoDB.");
        }
    }
