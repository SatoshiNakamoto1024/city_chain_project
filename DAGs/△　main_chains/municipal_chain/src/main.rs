...
use reqwest::Client;
use pqcrypto::sign::dilithium::{keypair, sign, verify};
use pqcrypto::encrypt::ntruprime::{generate_keypair, encrypt, decrypt};
use rsa::{RSAPrivateKey, RSAPublicKey, PaddingScheme, pkcs1::FromRsaPublicKey};
use rand::rngs::OsRng;
use rand::prelude::*;
...
use pqcrypto::sign::dilithium2; // 追加
use pqcrypto_dilithium::dilithium2::{
    keypair as generate_main_sign_keys, // Municipal Chain 用の鍵生成
    detached_sign as sign_transaction_with_main_key, // トランザクション署名
    verify_detached as verify_transaction_with_main_key, // 署名検証
    PublicKey as MainSignPublicKey, // 公開鍵の型
    SecretKey as MainSignPrivateKey, // 秘密鍵の型
    DetachedSignature as MainSignature, // 署名の型
};
use pqcrypto_ntruprime::sntrup761::{
    keypair as generate_main_param_keys,          // Municipal Chain 用の NTRU 鍵ペア生成
    encapsulate as encrypt_with_main_param_key,  // データ暗号化
    decapsulate as decrypt_with_main_param_key,  // データ復号化
    PublicKey as MainParamPublicKey,             // 公開鍵の型
    SecretKey as MainParamPrivateKey,            // 秘密鍵の型
    Ciphertext as MainParamCiphertext,           // 暗号化されたデータの型
};
use immudb_proto::{SetRequest, KeyValue};
use immudb_proto::GetRequest;
use immudb_proto::immudb_proto_function;
use prost::bytes::Bytes;
// immuDBのブロック / ウォレット分離に備えたKey作成用定数
const IMMUDB_BLOCK_PREFIX: &str = "block_";
const IMMUDB_WALLET_PREFIX: &str = "wallet_";

use std::collections::VecDeque;  // イベントを保持するため

// ★ 追加: ProofOfHistory (PoH) の簡易実装
...
impl ProofOfHistory {
    ...
    /// 新たなイベント（トランザクションIDやタイムスタンプなど）をPoHへ追加する
    fn add_event(&mut self, event_str: &str) {
        let mut hasher = Sha256::new();

        // 以前の current_hash と新しいeventを連結してハッシュ
        ...
        // 追加
        self.sequence.push_back(event_str.to_string());
        self.current_hash = new_hash_hex;
    }

    /// PoHチェーン最新ハッシュを返す
    fn get_latest_hash(&self) -> &str {
        &self.current_hash
    }
...
// ★ 変更ここから: まず、immuDBにウォレットを保存/取得/更新する関数を追加
use immudb_proto::immu_service_client::ImmuServiceClient;
use tonic::transport::Channel;

// 新しく「ウォレット用のKey」を作って immuDB に保存する関数
async fn save_wallet_to_immudb(
    client: &mut ImmuServiceClient<Channel>,
    user_id: &str,
    balance: f64
) -> Result<(), Box<dyn std::error::Error>> {
    let wallet_key = format!("{}{}", IMMUDB_WALLET_PREFIX, user_id); // "wallet_{user_id}" のキー
    let wallet_value = balance.to_string(); // balanceを文字列化
    let kv = KeyValue {
        key: wallet_key.into_bytes(),
        value: wallet_value.into_bytes(),
    };
    let request = tonic::Request::new(SetRequest { kvs: vec![kv] });
    client.set(request).await?;
    println!("Wallet data for user {} saved to immuDB (balance={}).", user_id, balance);
    Ok(())
}

// immuDB からウォレットのbalanceを取得する関数
async fn get_wallet_from_immudb(
    client: &mut ImmuServiceClient<Channel>,
    user_id: &str
) -> Result<f64, Box<dyn std::error::Error>> {
    let wallet_key = format!("{}{}", IMMUDB_WALLET_PREFIX, user_id);
    ...
    let balance_str = String::from_utf8(value)?;
    let balance = balance_str.parse::<f64>().unwrap_or(0.0);
    Ok(balance)
}

// ウォレットのバランスを更新
async fn update_wallet_in_immudb(
    client: &mut ImmuServiceClient<Channel>,
    user_id: &str,
    delta: f64
) -> Result<f64, Box<dyn std::error::Error>> {
    let current_balance = get_wallet_from_immudb(client, user_id).await?;
    let new_balance = current_balance + delta;
    // 下回り(負)にならないよう、最低0でクリップするならここで処理してもOK
    save_wallet_to_immudb(client, user_id, new_balance).await?;
    println!("Wallet for user {} updated in immuDB (delta={}, new={}).", user_id, delta, new_balance);
    Ok(new_balance)
}

pub struct MunicipalChain {
    // Dilithium (署名) の秘密鍵と公開鍵
    pub main_sign_public_key: dilithium2::PublicKey,
    pub main_sign_private_key: dilithium2::SecretKey,

    // NTRU (暗号化) の秘密鍵と公開鍵
    pub main_param_private_key: NtruPrivateKey,
    pub main_param_public_key: NtruPublicKey,

    // 他Municipal Chain の NTRU公開鍵
    partner_main_param_keys: HashMap<String, NtruPublicKey>,
}
...
impl MunicipalChain {
    pub fn new() -> Self {
        ...
        // NTRUキーの生成
        let (ntru_pk, ntru_sk) = ntru_keypair(); // 例

        MunicipalChain {
            main_sign_private_key: sk,
            main_sign_public_key: pk,
            main_param_private_key: ntru_sk,
            main_param_public_key: ntru_pk,
            partner_main_param_keys: HashMap::new(),
        }
    }

    // Dilithium公開鍵を取得するメソッド
    pub fn get_main_sign_public_key(&self) -> Vec<u8> {
        self.main_sign_public_key.as_bytes().to_vec()
    }

    // NTRU公開鍵を取得するメソッド
    pub fn get_main_param_public_key(&self) -> Vec<u8> {
        self.main_param_public_key.as_bytes().to_vec()
    }

    // 他Municipal ChainとのNTRU公開鍵の交換
    pub fn exchange_main_param_keys(&mut self, partner_id: String, partner_public_key_bytes: &[u8]) {
        let partner_public_key = NtruPublicKey::from_bytes(partner_public_key_bytes)
            .expect("Invalid NTRU public key");
        self.partner_main_param_keys.insert(partner_id, partner_public_key);
    }

    // トランザクションデータに署名を付加
    pub fn sign_transaction(&self, transaction_data: &[u8]) -> Vec<u8> {
        // `sign()` が署名(バイト列)を返す
        let signature = dilithium2::sign(transaction_data, &self.main_sign_private_key);
        signature
    }
    ...
    pub fn verify_transaction(
        ...
        let result = dilithium2::verify(signature, transaction_data, &self.main_sign_public_key);
        match result {
            Ok(()) => true,     // 署名OK
            Err(_) => false,    // NG
        }
    ...
    // 他Municipal Chainとの暗号化通信
    pub fn secure_communication_with_partner(
        &self,
        partner_id: &str,
        message: &[u8],
    ) -> Result<(Vec<u8>, Vec<u8>), Box<dyn Error>> {
        if let Some(partner_public_key) = self.partner_main_param_keys.get(partner_id) {
            let (ciphertext, shared_secret) = ntru_encapsulate(partner_public_key);
            ...
            Ok((ciphertext.as_bytes().to_vec(), encrypted_message))
        } else {
            Err("Partner NTRU public key not found".into())
        }
    }
    ...
    pub fn decrypt_message(
        ...
        let ciphertext = NtruCiphertext::from_bytes(ciphertext_bytes).map_err(|_| "Invalid ciphertext")?;
        let shared_secret = ntru_decapsulate(&ciphertext, &self.main_param_private_key)
...
fn sign(msg: &[u8], sk: &SecretKey) -> Vec<u8>
...
struct Transaction {
    ...
    attributes: HashMap<String, String>, // 動的な属性を保持するフィールド
}
...
struct AppState {
    ...
    pub wallet_collection: Collection<Document>, // 追加: ウォレットコレクション
...
impl AppState {
    ...
    // 既存の "get_wallet_balance" / "update_wallet_balance" でも immuDB を使うようにする
    async fn get_wallet_balance(&self, user_id: &str) -> Result<f64, Box<dyn std::error::Error>> {
        // MongoDB 側の残高
        let filter = doc! { "user_id": user_id };
        let mongo_balance = if let Some(doc) = self.wallet_collection.find_one(filter, None).await? {
            if let Some(b) = doc.get_f64("balance") {
                b
            } else {
                0.0
            }
        ...
        // immuDB 側の残高 (host/port は仮に "localhost:3322" など固定 or 設定ファイルで)
        let immudb_conf = load_immudb_config().expect("Failed to load immudb config");
        let (host, port) = get_immudb_address(&immudb_conf, "default"); // 必要に応じて市町村名を使う
        let mut client = ImmuServiceClient::connect(format!("http://{}:{}", host, port)).await?;
        let immu_balance = get_wallet_from_immudb(&mut client, user_id).await.unwrap_or(0.0);

        // 2重管理の場合、どちらを採用するかは要件次第。
        // ここでは「より大きい方」を返すサンプル (単に例)
        let final_balance = if immu_balance > mongo_balance {
            immu_balance
        } else {
            mongo_balance
        };
    ...
    async fn update_wallet_balance(&self, user_id: &str, amount: f64) -> Result<(), Box<dyn std::error::Error>> {
        // まず MongoDB 側を upsert
        let filter = doc! { "user_id": user_id };
        let update = doc! { "$inc": { "balance": amount } };
        let options = mongodb::options::UpdateOptions::builder().upsert(true).build();
        self.wallet_collection.update_one(filter, update, options).await?;

        // immuDB 側も更新 (host/portは適宜)
        let immudb_conf = load_immudb_config().expect("Failed to load immudb config");
        let (host, port) = get_immudb_address(&immudb_conf, "default");
        let mut client = ImmuServiceClient::connect(format!("http://{}:{}", host, port)).await?;
        let _new_balance = update_wallet_in_immudb(&mut client, user_id, amount).await?;
        ...
        if let Some(representative) = &self.approved_representative {
            ...
            if let Some(representatives) = self.municipal_representatives.get(&sender_municipality) {
                ...
                if sender_balance < transaction.amount {
                    println!("Insufficient balance for user: {}", transaction.sender);
                    return false;
                }

                // 残高更新（送信者分）
                state.update_wallet_balance(&transaction.sender, -transaction.amount).await?;

                // 残高更新（受信者分）
                state.update_wallet_balance(&transaction.receiver, transaction.amount).await?;
...
impl Transaction {
    ...
    fn verify_signature(&self, public_key: &DilithiumPublicKey) -> bool {
        // トランザクションデータをシリアライズ
        let serialized_data = format!("{:?}", self).as_bytes();

        // 署名検証
        match dilithium_verify(serialized_data, &self.signature, public_key) {
            Ok(_) => true,  // 検証成功
            Err(_) => false, // 検証失敗
        }
    }
...
#[post("/create_wallet", format = "json", data = "<req>")]
async fn create_wallet(req: Json<CreateWalletRequest>, state: &State<Arc<Mutex<AppState>>>) -> Status {
    let state_guard = state.lock().await;
    match state_guard.update_wallet_balance(&req.user_id, req.initial_balance).await {
        Ok(_) => Status::Ok,
        Err(_) => Status::InternalServerError
    }
}

#[get("/get_balance/<user_id>")]
async fn get_balance(user_id: &str, state: &State<Arc<Mutex<AppState>>>) -> Option<String> {
    let state_guard = state.lock().await;
    match state_guard.get_wallet_balance(user_id).await {
        Ok(balance) => Some(balance.to_string()),
        Err(_) => None,
    }
}

#[get("/get_balance/<wallet_address>")]
fn get_balance(wallet_address: String) -> String {
    match calculate_wallet_balance(&wallet_address) {
        Ok(balance) => format!("{{\"balance\": {}}}", balance),
        Err(_) => String::from("{\"balance\": 0}"),
    }
}
...
async fn create_transaction(
    ...
    let tx_data = serde_json::to_vec(&transaction).unwrap(); // Txをバイト列化
    let signature_bytes = municipal_chain.sign_transaction(&tx_data);
    transaction.signature = base64::encode(signature_bytes);
    ...
    let new_doc = doc! {
        ...
        "attributes": bson::to_bson(&transaction.attributes).unwrap_or(bson::Bson::Null), // 追加
    };
...
// 例: 受信したTransaction構造体を検証
fn verify_incoming_transaction(municipal_chain: &MunicipalChain, tx: &Transaction) -> bool {
    // Txのシリアライズ
    let tx_data = serde_json::to_vec(tx).unwrap();

    // base64エンコードされた署名をデコード
    let signature_bytes = match base64::decode(&tx.signature) {
        Ok(bytes) => bytes,
        Err(_) => return false,
    };

    // 検証実行
    municipal_chain.verify_transaction(&tx_data, &signature_bytes)
}
...
async fn approve_block(transaction_id: &str) -> Result<(), Box<dyn Error>> {
    ...
    // PoHに "block_index + ブロックハッシュ" などの文字列を追加
    {
        let mut state_guard = state.lock().await;
        let mut poh_guard = state_guard.poh.lock().await;
        let event_str = format!("block:{}:{}", new_block.index, new_block.hash);
        poh_guard.add_event(&event_str);
        println!("PoH updated with block event => latest_hash={}", poh_guard.get_latest_hash());
    }
...
#[get("/poh_status")]
async fn get_poh_status(state: &State<Arc<Mutex<AppState>>>) -> Json<Value> {
    let state_guard = state.lock().await;
    let poh_guard = state_guard.poh.lock().await;
    let events = poh_guard.get_events();
...
async fn continuous_poh_update(state: Arc<Mutex<AppState>>) {
    let mut interval = tokio::time::interval(Duration::from_millis(500)); // 0.5秒ごと
    loop {
        interval.tick().await;
        let now_str = Utc::now().to_rfc3339();
        {
            let mut state_guard = state.lock().await;
            let mut poh_guard = state_guard.poh.lock().await;
            let event_str = format!("tick:{}", now_str);
            poh_guard.add_event(&event_str);
            println!("[PoH thread] event={}, latest_hash={}", event_str, poh_guard.get_latest_hash());
        }
    }
}
...
async fn receive_transaction<'r>(
    ...
    // 必要なフィールドを取得
    let sender = transaction_json.get("sender").and_then(|v| v.as_str()).unwrap_or_default().to_string();
    let transaction_json = transaction_data.clone();
    ...
    {
        let mut state_guard = state.lock().await;
        let mut poh_guard = state_guard.poh.lock().await; // Arc<Mutex<ProofOfHistory>>想定
        let event_str = format!("tx:{}:{}", transaction.transaction_id, Utc::now().to_rfc3339());
        poh_guard.add_event(&event_str);
        println!("PoH updated => latest_hash={}", poh_guard.get_latest_hash());
    }

    // 暗号化されたデータと署名を取得
    let encrypted_data = match transaction_data.get("encrypted_transaction").and_then(|v| v.as_str()) {
        Some(data) => data,
        None => {
            println!("Missing encrypted transaction data");
            return Err(Status::BadRequest);
        }
    };

    let signature = match transaction_data.get("signature").and_then(|v| v.as_str()) {
        Some(sig) => sig,
        None => {
            println!("Missing transaction signature");
            return Err(Status::BadRequest);
        }
    };
    ...
    // 復号化
    let state_guard = state.lock().await;
    let ntru_private_key = &state_guard.main_param_private_key;
    let decrypted_data = match decrypt_transaction_data(encrypted_data.as_bytes(), ntru_private_key) {
        Ok(data) => data,
        Err(_) => {
            println!("Failed to decrypt transaction data");
            return Err(Status::Unauthorized);
        }
    };

    // 署名検証
    let dilithium_public_key = &state_guard.main_sign_public_key;
    if !verify_signature(&decrypted_data, signature.as_bytes(), dilithium_public_key) {
        println!("Signature verification failed");
        return Err(Status::Unauthorized);
    }

    // JSONデータからトランザクションを生成
    let verified_transaction: Transaction = match serde_json::from_str(&decrypted_data) {
        Ok(tx) => tx,
        Err(_) => {
            println!("Failed to parse transaction data");
            return Err(Status::UnprocessableEntity);
        }
    };

    println!("Transaction successfully verified: {:?}", verified_transaction);
    // `verified_transaction` から必要なフィールドを取得
    ...
    if let Ok(json_value) = transaction {
        ...
        if let (
            ...
            json_value.get("to_wallet").and_then(|sr| sr.as_str()),
            json_value.get("from_wallet").and_then(|tr| tr.as_str()),
            json_value.get("amount").and_then(|rr| rr.as_str()),
            ...
            json_value.get("attributes").and_then(|cr| cr.as_str()),
        ) {
            ...
            match (parse_timestamp(timestamp_str), parse_timestamp(created_at_str)) {
                (Ok(parsed_timestamp), Ok(parsed_created_at)) => {
                    ...
                    let transaction = Transaction {
                        ...
                        attributes: transaction.attributes.clone(), // 追加
                    };
...
// トランザクションに署名を追加する関数
fn sign_transaction(transaction: &mut Transaction, private_key: &DilithiumSecretKey) {
    // トランザクションデータを署名用メッセージとして生成
    let message = format!(
        "{:?}{:?}{:?}{:?}{:?}{:?}{:?}{:?}",
        transaction.sender,
        ...
        transaction.subject,
        transaction.action_level,
        transaction.dimension,
        transaction.organism_name
    );

    // Dilithiumの秘密鍵を使用して署名を生成
    let signature = dilithium_sign(message.as_bytes(), private_key);

    // 署名をトランザクションのフィールドにエンコードして格納
    transaction.signature = base64::encode(signature);
}

// 署名を検証する関数
fn verify_transaction_signature(transaction: &Transaction, public_key: &DilithiumPublicKey) -> bool {
    // トランザクションデータを署名検証用メッセージとして生成
    let message = format!(
        "{:?}{:?}{:?}{:?}{:?}{:?}{:?}{:?}",
        transaction.sender,
        transaction.receiver,
    ...
    let signature_bytes = match base64::decode(&transaction.signature) {
        Ok(bytes) => bytes,
        Err(_) => return false, // デコードエラー時は署名検証失敗
    };

    // Dilithium公開鍵を使用して署名を検証
    dilithium_verify(message.as_bytes(), &signature_bytes, public_key).is_ok()
}

// トランザクションデータを暗号化する関数
pub fn encrypt_transaction_data(transaction_data: &[u8], main_param_public_key: &[u8]) -> Vec<u8> {
    let serialized_transaction = serde_json::to_string(transaction).unwrap();
    public_key.encrypt(&serialized_transaction.as_bytes())
    let encrypted = ntru_encrypt(transaction_data, main_param_public_key);
    encrypted
}

// トランザクションデータを復号化する関数
pub fn decrypt_transaction_data(encrypted_data: &[u8], main_param_private_key: &[u8]) -> Vec<u8> {
    let decrypted = ntru_decrypt(encrypted_data, main_param_private_key);
    decrypted
...
async fn save_block_to_immudb(client: &mut ImmuServiceClient<Channel>, block: &Block) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let block_key = format!("{}{}", IMMUDB_BLOCK_PREFIX, block.index);
    let kv = KeyValue {
...
async fn get_block_from_immudb(client: &mut ImmuServiceClient<Channel>, block_index: u64) -> Result<Block, Box<dyn std::error::Error>> {
    let block_key = format!("{}{}", IMMUDB_BLOCK_PREFIX, block_index);
    let request = tonic::Request::new(GetRequest {
        key: block_key.into_bytes(),
        at_tx: 0,
        since_tx: 0,
...
use tokio::{task, time::{interval, Duration}};
use rocket::fairing::AdHoc;
...
fn rocket() -> Rocket {
    rocket::ignite().mount("/", routes![get_balance])
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // main() で continuous_poh_update を起動
    tokio::spawn({
        let state_clone = Arc::clone(&state);
        async move {
            continuous_poh_update(state_clone).await;
        }
    });
    ...
    let client_options = ClientOptions::parse(&mongo_uri).await.expect("MongoDB client options failed");
    let mongo_client = MongoClient::with_options(client_options).expect("MongoDB client failed");
    let mongo_collection = mongo_client.database(&mongo_db_name).collection::<mongodb::bson::Document>("pending_transactions");
    ...
    // analytics_db_name を生成
    let analytics_db_name = {
        let uri_parts: Vec<&str> = analytics_uri.split('/').collect();
        uri_parts.last().unwrap_or("analytics_db").to_string()
    };

    let analytics_collection = analytics_client.database(&analytics_db_name).collection::<mongodb::bson::Document>("shared_transactions");

    // Municipal Chain 用の鍵ペア生成
    let (main_sign_public_key, main_sign_private_key) = generate_main_sign_keys();
    let (main_param_public_key, main_param_private_key) = generate_main_param_keys();

    // 暗号化と復号化のテスト
    let transaction_data = b"example transaction data";
    let (main_param_ciphertext, shared_secret) = encrypt_with_main_param_key(&main_param_public_key);
    let recovered_shared_secret = decrypt_with_main_param_key(&main_param_ciphertext, &main_param_private_key);
    println!("Shared secret match: {}", shared_secret == recovered_shared_secret);
    ...
    task::spawn(async move {
        approve_and_send_transactions(state_clone, client_clone).await;
    });
...
mod tests {
    ...
    async fn test_update_status() {
        ...
        let test_transaction = Transaction::new(
            ...
            "sender_municipal_id".to_string(),
            "receiver_municipal_id".to_string(),
            "attributes".attributes.clone(), // 新しく追加
        );
        ...
        let new_doc = doc! {
            ...
            "to_wallet": &test_transaction.to_wallet,
            "from_wallet": &test_transaction.from_wallet,
            ...
            "attributes": &test_transaction.attributes,
        };
